...
// WIP - moving a subset from ugate, for SNI sniffing and routing.
...
func (s *Stream) Fill(i int) ([]byte, error) {
	for {
		n, err := s.in.Read(s.buf[s.rend:])
		s.rend += n
		if err != nil {
			s.rErr = err
...
func ParseTLS(acc *Stream) (string,error) {
	buf, err := acc.Fill(5)
	if err != nil {
		return "", err
	}
